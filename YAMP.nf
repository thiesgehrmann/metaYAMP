...
process decontaminate {
    ...
    bbwrap.sh -Xmx\"\$maxmem\" \\
        ...
        qin=$params.qin \\
        threads=${task.cpus} \\
        untrim quickmatch fast ow &> decontamination_mqc.txt
...
process merge_paired_end_cleaned {
    ...
    //Enable multicontainer settings
    // as per humann guidelines, we do not merge the reads with alignment, but rather concatenate the read files.
    // https://github.com/biobakery/humann#humann-30-and-paired-end-sequencing-data
    //if (workflow.containerEngine == 'singularity') {
    //    container params.singularity_container_pear
    //} else {
    //    container params.docker_container_pear
    //}
    ...
    output:
    tuple val("${name}.merged"), path("${name}.QCd.merged.fq.gz") into to_profile_taxa_merged
    tuple val("${name}.merged"), path("${name}.QCd.merged.fq.gz") into to_profile_functions_merged
    ...
    if [ -z "${reads[1]}" ]; then
        if (file -L ${reads[0]} | grep -q compressed ) ; then
            ln -s ${reads[0]} "${name}.QCd.merged.fq.gz"
        else
            cat ${reads[0]} | gzip > "${name}.QCd.merged.fq.gz"
        fi
        
        #ln -s ${name}_QCd.fq.gz ${name}_QCd.assembled.fq.gz
        #ln -s ${name}_QCd.fq.gz ${name}_QCd.unassembled.fq.gz
        #ln -s ${name}_QCd.fq.gz ${name}_QCd.all.fq.gz
    else
        if (file -L ${reads[0]} | grep -q compressed ) ; then
            #pear \\
            #    -f <( zcat ${reads[0]}) \\
            #    -r <( zcat ${reads[1]}) \\
            #    -o "${prefix}"
            cat "${reads[0]}" "${reads[1]}" > "${name}.QCd.merged.fq.gz"
        else
            #pear \\
            #    -f ${reads[0]} \\
            #    -r ${reads[1]} \\
            #    -o ${prefix}
            cat "${reads[0]}" "${reads[1]}" | gzip > "${name}.QCd.merged.fq.gz"
        fi

        #cat ${prefix}.assembled.fastq | gzip > ${name}_QCd.assembled.fq.gz
        #cat ${prefix}.unassembled.forward.fastq ${prefix}.unassembled.reverse.fastq | gzip > ${name}_QCd.unassembled.fq.gz
        #cat ${prefix}.assembled.fastq ${prefix}.unassembled.forward.fastq ${prefix}.unassembled.reverse.fastq | gzip > ${name}_QCd.all.fq.gz
    fi
    """
}
...
process profile_taxa {
    ...
    //Enable multicontainer settings
    if (workflow.containerEngine == 'singularity') {
        container params.singularity_container_metaphlan
    } else {
    ...
    metaphlan --input_type fastq -x mpa_vJan21_CHOCOPhlAnSGB_202103 --offline --tmp_dir=. --biom ${name}.biom --bowtie2out=${name}_bt2out.txt --bowtie2db $bowtie2db --bt2_ps ${params.bt2options} --add_viruses --sample_id ${name} --nproc ${task.cpus} $reads ${name}_metaphlan_bugs_list.tsv &> profile_taxa_mqc.txt
...
process profile_function {
    ...
    //Enable multicontainer settings
    if (workflow.containerEngine == 'singularity') {
        container params.singularity_container_humann
    } else {
        container params.docker_container_humann
    }
    ...
    humann --input $reads --output . --output-basename ${name} --taxonomic-profile $metaphlan_bug_list --nucleotide-database $chocophlan/chocophlan --protein-database $uniref/uniref --pathways metacyc --threads ${task.cpus} --memory-use minimum &> ${name}_HUMAnN.log 
